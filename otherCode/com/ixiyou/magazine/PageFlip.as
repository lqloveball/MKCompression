	...
	public class PageFlip {
		...
		*/
		...
		public static function computeFlip(ptd:Point,pt:Point,pw:int,ph:int,ish:Boolean,sens:int):Object{
			...
			var dragPt:Point
			var show1Arr:Array
			var show1Mtr:Matrix = new Matrix()
			var tempW:Number 
			if (cPoints) {
				dragPt = cPoints[0] as Point
				angle = ArcMath.angleOfLine(cPoints[cPoints.length-1].x, cPoints[cPoints.length-1].y, cPoints[cPoints.length-2].x, cPoints[cPoints.length-2].y)-90
				if (pt.x == 0 && pt.y == 0) {
					pt=new Point(0,0)
					cLine = ArcMath.distance(dragPt.x, dragPt.y, 0, 0)
					if (cPoints.length == 3) show1Arr = [pt, cPoints[1], cPoints[2]];
					else show1Arr = show1Arr = [pt, cPoints[3], cPoints[2], new Point(0, oph)];
				}
				else if (pt.x == 1 && pt.y == 0) {
					pt=new Point(opw,0)
					cLine = ArcMath.distance(dragPt.x, dragPt.y, opw, 0)
					if (cPoints.length == 3) show1Arr = [pt, cPoints[1], cPoints[2]]
					else show1Arr = show1Arr = [pt, cPoints[3], cPoints[2], new Point(opw, oph)]
					angle-=180
				}
				else if (pt.x == 0 && pt.y == 1) {
					pt=new Point(0,oph)
					cLine = ArcMath.distance(dragPt.x, dragPt.y, 0, oph)
					if (cPoints.length == 3) show1Arr = [pt, cPoints[1], cPoints[2]]
					else show1Arr = show1Arr = [pt, cPoints[3], cPoints[2], new Point(0, 0)]
					angle-=180
				}
				else {
					pt=new Point(opw,oph)
					cLine = ArcMath.distance(dragPt.x, dragPt.y, opw, oph)
					if (cPoints.length == 3) show1Arr = [pt, cPoints[1], cPoints[2]]
					else show1Arr= show1Arr = [pt,cPoints[3], cPoints[2],new Point(opw,0)]
				}
				show1Mtr.createGradientBox(
					ArcMath.distance(cPoints[2].x, cPoints[0].y, pt.x, pt.y),
					oph,
					ArcMath.degreesToRadians(angle),
					-opw*0.5,-oph*0.5
				); 
				//angle = ArcMath.fixAngle(angle)
			}
			
			// now we just have to return all the stuff
			return {show1Mtr:show1Mtr,show1Arr:show1Arr,angle:angle,dragPt:dragPt,pt:pt,cLine:cLine,cPoints:cPoints,pPoints:pPoints,matrix:mat,width:opw,height:oph};
		}
